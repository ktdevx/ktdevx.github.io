...
この時、**IPv6環境下稀にDHCPv6の設定後にインストーラーが止まる**場合があります。

私の環境では、DHCPv6のInformation Requestで応答が返ってこなかったことで、インストーラーが停止していたようです。ルーターのIPv6の設定を見直して解決しました。

ブルースクリーンとなってインストーラーが停止した場合は、ルーターのIPv6の設定を見直す、一時的にIPv4のみにする、ネットワークの自動設定中のみLANケーブルを抜いておくなど、適宜対応しましょう。
